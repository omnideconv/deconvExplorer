test_that("Signature exploration plots are correct", {
  ...
  p_sig_clust <- plot_signatureClustered(signature = signature_list$bisque, 
                                score = "gini", annotation_type = "bar")
  
  expect_s4_class(p_sig_clust, "HeatmapList")
  expect_error(
    plot_signatureClustered(signature = signature_list, 
                            score = "gini", annotation_type = "bar")
  )
  ...
  sig_vec <- download_signatureUpset(signatures = siglist2, 
